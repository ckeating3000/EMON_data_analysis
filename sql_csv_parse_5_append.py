...
#        looks for files in subdirectory titled "files_to_add"
...
all_files=[]
for path, subdirs, files in os.walk("./files_to_add/"):
    for name in files:
        all_files.append(os.path.join(path, name))
#all_files=os.listdir('./files_to_add/')
...
MeterID_to_Unit = {    # 749
                    ...
                    #*** Correction to raw schematic: I'm almost positive that G3L2-2A1 should be "3S" and not 2S as listed in raw data.  This fixes error where 2S has two "GenRec" monitors and gives 3S what is presumably its rightful GenRec monitor
...
MeterID_to_Subsys ={    # 749
                        ...
                        'G2L2-2A1':'Kitchen' , 'G2L2-2A2':'Lights' , 'G2L2-2A3':'GenRec'  , 'G2L2-2A4':'Spare' , 'G2L2-2A5':'Spare'     , 'G2L2-2A6':'Kitchen'   , 'G2L2-2A7':'Lights'    , 'G2L2-2A8':'GenRec'    ,
                        ...
                        'G2L3-3B1':'Laundry' , 'G2L3-3B2':'HVAC'   , 'G2L3-3B3':'Laundry' , 'G2L3-3B4':'HVAC'   , 'G2L3-3B5':'Microwave_N' , 'G2L3-3B6':'MakeupAir_N' , 'G2L3-3B7':'Microwave_S' , 'G2L3-3B8':'MakeupAir_S' ,
                        # 745
...
# check if full_list already contains the line; if not, add it
with open(master_name, 'r+') as myfile:
...
# Fix sorting so that year is prioritized
#full_list.sort(key=lambda x: (x.date_time.split('/')[2].split(" ")[0], x.datetime, x.building, x.unit, x.monitor_type), reverse=False)
